...
REGENERATING REWRITES ~545 FILES. Three of the four things it used to revert
are fixed at the source here, on 24 Aug 2026, after recording one company
turned into a repair job:

    the footer IA        FIXED - the template emits the Platforms column,
                         matching site_nav.py. It used to revert to "Work" and
                         drop the Software link, and site_nav could NOT see
                         it: its detector greps for the Platforms column
                         heading, which the reverted column does not have. It
                         reported "0 rewritten" over 543 broken footers.
                         (This comment says "the Platforms column heading"
                         rather than the tag itself for the same reason - the
                         detector reads this file, and a literal tag in prose
                         is the match it finds first.)
    Dataset JSON-LD      FIXED - generated by dataset_jsonld() from the same
                         counts the page header prints, instead of being
                         hand-written into suppliers.html where every
                         regeneration dropped it and refused the next build.
    consent.js           FIXED - the page template carries the tag. It had been
                         dropped from all 583 generated pages, removing cookie
                         consent from an EU entity's site, and it was not even
                         on this list: no gate caught it, because build_check
                         asserts analytics loads only FROM consent.js, never
                         that consent.js is present.

    prerendered blocks   STILL REVERTS. Regenerating strips them, taking the
                         register from 24,330 readable characters to 0. Run
                         research/prerender_instruments.py --apply afterwards.
                         build_check refuses the build if you forget.

So: regenerate, run prerender_instruments.py --apply, then read build_check
before committing. The lesson the first three cost is that a sweep which fixes
pages after the fact leaves the generator wrong, and the next person to add a
company pays for it.
"""
...
# Echoed from the data, never invented here - dpp_dataset_export.py reads the
# same key for the CSV and JSON citation. Set by load().
GENERATED = ""
      ...
      <div class="main-column">
        ...
        <section class="claim-card">
          ...
          <div class="success" role="status" id="claimSuccess" hidden>
            ...
            <div>
              <p class="success-title" id="successTitle">Request received</p>
              <p id="successBody"></p>
              <p id="successHelp"></p>
              <p id="successNext"></p>
            </div>
...
(function(){
  ...
  f.addEventListener('submit',function(ev){
    ...
    fetch('/api/claim',{method:'POST',headers:{'Content-Type':'application/json'},
      ...
      .then(function(r){
        ...
        //   1. STILL CONDITIONAL. It must never confirm that a domain matched.
        //      api/claim.js answers {ok:true} to every claim for the same reason:
        //      otherwise this form is a way to test who works where, one address
        //      at a time, against 191 organisations.
        //   2. A TIME BOUND. "On its way" with no duration is exactly what left
        ...
        // removed rather than reworded.
        {
          ...
          // Assembled, not written. Every other appearance of this address on the
          // site is built at runtime so it is not sitting in the HTML for a
          // harvester, and the copy above must not be the one exception.
          help.innerHTML = 'If no email arrives, whether because delivery was delayed or '
            + domain + ' is not the domain on record, email '
            + '<a href="#" id="successMail">hello' + String.fromCharCode(64) + 'yellow3.io</a>'
            + ' and we will help you access the editor.';
          next.textContent = 'The link opens the Company Information Editor, where you can review '
            + 'and update the company information. The public profile will not change until you '
            + 'submit the information there.';
          var m = document.getElementById('successMail');
          if (m) m.addEventListener('click', mailSupport);
        }
  ...
  // One builder for both routes in and out of the success state. The address is
  // assembled rather than written, same as everywhere else on the site, so it is
  // not sitting in the HTML for a scraper.
  function mailSupport(ev){
  ...
  document.getElementById('claimSupport').addEventListener('click', mailSupport);
})();
</script>
"""
...
# ---------------------------------------------------------------- the Dataset
# WHY THIS IS GENERATED RATHER THAN WRITTEN INTO THE PAGE.
#
# It used to be hand-added to suppliers.html, so every regeneration silently
# dropped it - the header note at the top of this file has listed that as a
# known casualty for weeks, and build_check refuses the build when it goes
# missing. Adding a company meant regenerating, which meant losing the schema
# that makes the register citable, which meant a refused build every time.
# Generating it ends that loop.
#
# EVERY VALUE COMES FROM THE DATA. The counts are the same ones the page header
# prints, and `generated` is echoed from dpp-suppliers.json exactly as
# dpp_dataset_export.py echoes it into the CSV and JSON citation - so the three
# published artifacts cannot disagree about what this dataset is.
def dataset_jsonld(counts, generated):
    doc = {
        "@context": "https://schema.org",
        "@type": "Dataset",
        "name": "DPP Supplier Register",
        "description": "An independent public register of organisations offering "
                       "Digital Product Passport capability, compiled by yellow3 lab "
                       "from public sources. Suppliers cannot pay to appear, to rank "
                       "higher, or to change an assessment.",
        "url": "https://www.yellow3.io/research/digital-product-passport/suppliers",
        ...
        "publisher": {"@type": "Organization", "name": "yellow3 lab",
                      "url": "https://www.yellow3.io"},
        "spatialCoverage": f"{counts['countries']} countries",
        "variableMeasured": [
            "Organisation name", "Headquarters country", "Entity type",
            "Sectors served", "Ownership", "Funding stage", "Source of record",
            "Date checked", "Confidence"],
        "distribution": [
            {"@type": "DataDownload", "encodingFormat": "text/csv",
             "contentUrl": "https://www.yellow3.io/research/digital-product-passport/suppliers.csv"},
            {"@type": "DataDownload", "encodingFormat": "application/json",
             "contentUrl": "https://www.yellow3.io/research/digital-product-passport/suppliers.json"}],
        "citation": f"yellow3 lab, DPP Supplier Register, data generated {generated}",
    }
...
""" + SITE_FOOTER
    ...
    out = page("Supplier directory - DPP Supplier Register - yellow3",
                ...
                body, script,
                og_extra=dataset_jsonld(counts, GENERATED))
