...
<body>
  <header class="gate">
    <a href="/" class="gate-brand"><img src="/logo.png" alt="yellow3" /></a>
  ...
  <div class="preview">
    <div class="preview-label">A live preview of the Professional dashboard &middot; full access unlocks at launch</div>
...
.gate-feats{display:flex;flex-wrap:wrap;gap:8px;justify-content:center;margin:24px 0}
.gate-feats span{font-size:12px;font-weight:600;color:var(--body);background:var(--panel);border:1px solid var(--line);padding:7px 13px;border-radius:999px}
.gate-price{font-size:19px;margin:28px 0 20px}.gate-price b{color:var(--asia)}.gate-price span{font-size:14px;color:var(--muted)}
.gate-cta{display:inline-flex;align-items:center;gap:10px;background:var(--ink);color:#fff;font-size:14px;font-weight:600;letter-spacing:.04em;text-transform:uppercase;padding:16px 30px;text-decoration:none}
.gate-cta:hover{background:#000}
.gate-note{font-size:12px;color:var(--muted);margin-top:22px}
.preview{position:relative;max-height:660px;overflow:hidden;border-top:1px solid var(--line)}
.preview-label{text-align:center;font-size:11px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:600;padding:22px 0 6px}
.locked{filter:blur(5px);opacity:.5;pointer-events:none;user-select:none}
.preview::after{content:"";position:absolute;left:0;right:0;bottom:0;height:220px;background:linear-gradient(transparent,#fff)}
